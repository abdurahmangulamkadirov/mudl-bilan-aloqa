# coding: utf-8
#Kodni yaxshiroq yozishga vaqt bo'lmadi
#Kod parsing orqali ma'lumotlarni oladi, yaxshi yo'lmas
#Yaxshisi APIni ochtirib, o'sha bilan ishlash kerak, ishonchli bo'ladi shunda
import sys
import requests
import os
import json
...
class Moodle:
	...
	def core_user_get_username(self, username, password):
		...
		if page.css_first('title').text() == "TATUFF Masofaviy ta'lim: Вход на сайт":
			if not self.core_auth_confirm_user(username, password):
				return ""#agar sessiya bekor qilinsa qayta avtorizatsiya so'rovi yuborilishi kerak
				#todo buni tezligini biroz oshirish uchun taym bilan ishlash mumkin
				#sessiyani saqlab qolishni bilmadim
			page = HTMLParser(self.session.get("http://moodle.fbtuit.uz/my/").text)
		'''
	...
	def core_task_get_info(self, username, password, task_id):
		...
		for div in page.css("div"):
			if div.attributes.get("id", '')=="page":
				infos['course_name'] = div.css_first("h1").text()#bu yaxshi yo'lmas, sayt dizaynida shrift hajmi o'zgarsa
				#xatolik beradi
		links = []
		submitted_files = []
		page.unwrap_tags(['p'])
	...
	def core_task_get_files(self, username, password, task_id, submission):
		...
		return 0
	#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	#0.3-versiya -- Kalendardagi topshiriqlarni ko'rsatish qismi qo'shiladi nasib qilsa :)
